// ==UserScript==
// @name         Onshape Metrics v3.3 - Firefox (GPU CORRIGIDO)
// @namespace    http://tampermonkey.net/
// @version      3.3
// @description  Onshape Firefox: WebGL GPU CORRIGIDO (GT 710), frame time, memory. Draggable.
// @author       Lexluthor
...
(function() {
  ...
  console.log('%c[ONSHAPE METRICS v3.3 - IMPORTANTE]%c', 'color: #FFD700; font-weight: bold; font-size: 14px;', '');
  console.log('%cEste script foi testado e otimizado para:%c', 'color: #00BCD4; font-weight: bold;', '');
  console.log('%cFirefox 115.25+ ESR (64-bit)%cSistema: Windows 10/11 64-bit%cGPU: NVIDIA GeForce GT 710%cCPU: Quad-core%cRAM: 8GB+%c', 'color: #4CAF50; font-size: 11px;', 'color: #4CAF50;', 'color: #4CAF50;', 'color: #4CAF50;', 'color: #4CAF50;', '');
  console.log('%c⚠️ Se você está usando uma versão diferente de Firefox, alguns dados podem não ser coletados corretamente.%c', 'color: #FF9800; font-weight: bold;', '');
  console.log('%c💡 Solução: Abra about:support e procure por "GPU #1 > Descrição" para confirmar sua GPU.%c', 'color: #2196F3;', '');
  console.log('%c📝 Se encontrar problemas, compartilhe seus dados do about:support para melhorar o script.%c', 'color: #E91E63;', '');
  ...
  function extractGPUFromANGLE(rendererString) {
    if (!rendererString) return 'Unknown';
    var match = rendererString.match(/NVIDIA\s+GeForce\s+[^D]*/);
    if (match) {
      return match[0].trim();
    }
    var match2 = rendererString.match(/,\s*([^,]+)\s+Direct3D/);
    if (match2) {
  ...
  function getWebGLInfo() {
    try {
      ...
      if (debugInfo) {
        ...
        var rawRenderer = gl.getParameter(debugInfo.UNMASKED_RENDERER_WEBGL);
        
        if (rawRenderer && rawRenderer.indexOf('ANGLE') !== -1) {
          renderer = extractGPUFromANGLE(rawRenderer);
          console.log('[Onshape] Extracted GPU from ANGLE:', renderer);
        } else {
  ...
  function createUI() {
    ...
    container.style.cssText = 'position: fixed; top: 20px; right: 20px; z-index: 99999; background: rgba(0,0,0,0.95); color: #fff; padding: 0; border-radius: 8px; font-family: monospace; font-size: 11px; box-shadow: 0 0 15px rgba(0,0,0,0.8); max-width: 340px; max-height: 400px; overflow-y: auto; border: 2px solid #00BCD4;';
  ...
  function updateUI() {
    ...
    if (info && window.OnshapeMetricsTM.samples.length > 0) {
      ...
      info.innerHTML = '<strong>Última amostra:</strong><br>FPS: ' + (last.fps || 'N/A') + '<br>Frame Time: ' + (last.frame_time_ms || 'N/A') + 'ms<br>GPU: ' + (last.webgl_renderer || 'N/A') + '<br>Memory: ' + last.memory_used_mb + ' / ' + last.memory_limit_mb + ' MB<br>Interaction: ' + (last.interaction_latency_ms || 'N/A') + 'ms';
    }
  }
  ...
  window.addEventListener('load', function() {
    setTimeout(function() {
      ...
      console.log('[Onshape Metrics v3.3 Firefox] Ready - GPU FIXED');
    }, 2000);
  });
